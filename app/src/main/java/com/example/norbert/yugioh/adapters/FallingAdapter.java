...
public class FallingAdapter extends BaseCardAdapter {

    public FallingAdapter(Context context, List<Card> cards) {
        super(context, cards);
    }
    ...
    protected String getCardPrice(Card card) {
        return "Price: $" + String.valueOf(df.format(Double.parseDouble(card.getPrice()))) +
                "     Shift: " + String.valueOf(df.format(Double.parseDouble(card.getPrice_shift()) * 100)) + "%";
    }
}
